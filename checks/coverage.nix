...
let
  ...
  waived = {
    ...
    "FW-4" = "testable today in a VM -- `nft flush ruleset`, then assert nixnet's table is back within one reconcile interval -- but the repair loop it tests does not exist yet";
    ...
    "TF-2" = "publish-on-reconcile is the defect that made half this project fiction, and it has no VM test: the shape is to change the live route out from under a settled daemon and assert it is re-asserted within one interval";
    "TF-3" = "loser demotion needs two real uplinks in one VM and an assertion that no two metrics tie; not written";
